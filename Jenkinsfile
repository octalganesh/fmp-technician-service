                        ...
                        withCredentials([usernamePassword(credentialsId: '46957a41-b9d8-40ec-8b21-3b41ecca86b9', usernameVariable: 'DEPLOY_USER', passwordVariable: 'DEPLOY_PASS')]) {
                            sh '''
                                ...
                                sshpass -p "$DEPLOY_PASS" ssh -o StrictHostKeyChecking=no $DEPLOY_USER@$DEPLOY_HOST "bash -s" << 'EOF'
                                    set -e
                                    mkdir -p /opt/apps/technician-service
                                    chmod 775 /opt/apps/technician-service

                                    echo "Stopping old app..."
                                    pkill -f technician-service-0.0.1-SNAPSHOT.jar || true

                                    echo "Starting new app..."
                                    nohup java -jar /opt/apps/technician-service/technician-service-0.0.1-SNAPSHOT.jar \
                                        > /opt/apps/technician-service/technician-service.log 2>&1 &

                                    sleep 5
                                    PID=$(pgrep -f technician-service-0.0.1-SNAPSHOT.jar || true)
